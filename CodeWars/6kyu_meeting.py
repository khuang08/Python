def meeting(s):
    names = s.upper().strip(';').split(';')

    names = [tuple(x.split(':')) for x in names]
    names.sort(key=lambda tup: (tup[1], tup[0]))

    names = [(name[1], name[0]) for name in names]

    names = str(''.join(str(x) for x in names)).replace("'","")
...
print(meeting("Anna:Wahl;Grace:Gates;James:Russell;Elizabeth:Rudd;Victoria:STAN;Jacob:Wahl;Alex:Wahl;Antony:Gates;Alissa:Meta;Megan:Bell;Amandy:Stan;Anna:Steve"))
